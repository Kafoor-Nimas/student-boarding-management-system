document.addEventListener('DOMContentLoaded', function() {
    ...
    const listingsTableBody = document.getElementById('listingsTableBody') || document.getElementById('listingTable');
    ...
    async function loadListingsFromDB() {
        ...
        try {
            ...
            listingsTableBody.innerHTML = ''; // Clear table

            listings.forEach(listing => {
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td>
                        <div class="listing-title"><strong>${listing.listing_title}</strong></div>
                        <small>${listing.location}</small>
                    </td>
                    <td>${listing.university}</td>
                    <td>LKR ${parseFloat(listing.monthly_price).toLocaleString()}</td>
                    <td><span class="status-badge status-available">${listing.status || 'Active'}</span></td>
                    <td><span class="rating">New</span></td>
                    <td>
                        <div class="action-buttons">
                            <button class="action-btn" title="View Details"><i class="fas fa-file-alt"></i></button>
                            <button class="action-btn" title="Delete" onclick="deleteListing(${listing.id})" style="color:red">
